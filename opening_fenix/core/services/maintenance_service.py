...
from opening_fenix.core.services.lichess_service import run_lichess_import
...
def list_all_repertoires():
    ...
    return [{'name': n, 'elo': get_repertoire_elo(n)} for n in unique_names]
...
class MaintenanceOrchestrator:
    def __init__(self, repo_configs, tasks, engine_settings, 
        ...
        self.stats_pending = {cfg['name']: {'engine': False, 'lichess': False} for cfg in repo_configs}
    ...
    def _mark_task_done(self, name, task_type):
        with self.stats_lock:
            ...
            ready_for_stats = self.stats_pending[name]['engine'] and self.stats_pending[name]['lichess']
            
        if ready_for_stats and self.tasks.get('stats') and not self._is_aborted:
            self.repo_status_cb(name, "stats", 0, "Statistiken...")
            try:
                calculate_priority_scores(name)
